(function (global, factory) {
  ...
  function renderTextComponents(components, textSequences, colorSequences, sequenceIndex, tokenSlug, previewMode, selectedComponentType, selectedComponentId) {
    return components.map(function (component) {
      ...
      var textBounds = measureTextBounds(value, component.fontFamily, component.fontWeight, fontSize);
  ...
  function getTextMetrics(text, fontFamily, fontWeight, fontSize) {
    var bounds = measureTextBounds(text, fontFamily, fontWeight, fontSize);
    if (bounds) {
      return {
        width: bounds.width,
        ascent: Math.max(0, -bounds.y),
        descent: Math.max(0, bounds.y + bounds.height),
  ...
  function measureTextBounds(text, fontFamily, fontWeight, fontSize) {
    ensureMeasureTextNode();
    if (!measureText || !text || !fontSize) {
      return null;
    }
    ...
    return measureText.getBBox();
  }

  function ensureMeasureTextNode() {
    if (measureText || typeof document === "undefined" || !document.body) {
      return;
    }
    ...
    measureText.setAttribute("font-style", "normal");
