(function (global, factory) {
  ...
  function createAppView(appElement) {
    appElement.innerHTML = [
      ...
      '        <button class="menu-button" type="button" data-action="load-release-example" aria-label="Load Latest Example" title="Load Latest Example"><span aria-hidden="true">Ex</span></button>',
      ...
      '      <div class="app-menu-group app-menu-group-external">',
      '        <a class="menu-link menu-link-github" href="' + GITHUB_REPO_URL + '" target="_blank" rel="noopener noreferrer" aria-label="GitHub Repository" title="GitHub Repository">' + renderGitHubIcon() + "</a>",
      "      </div>",
  ...
  function renderHelpDialog() {
    return [
      ...
      '      <h2 id="help-title">Help (' + escapeHtml(APP_VERSION) + ")</h2>",
      ...
      '      <h3>Printer Setup</h3>',
      '      <ul class="help-list">',
      '        <li>Match the browser print dialog paper size and orientation to the app&apos;s Print Settings.</li>',
      '        <li>Print at <strong>Actual size</strong> or <strong>100%</strong>. Turn off Fit to page, Shrink to fit, or any other scaling.</li>',
      '        <li>Enable background graphics/background colors if your browser offers that option.</li>',
      '        <li>Use the app&apos;s margin setting for spacing; keep extra browser/printer margins off if possible.</li>',
      '        <li>Before sending a full run to a printer, print one page and measure a 1&quot; token with a ruler or calipers.</li>',
      "      </ul>",
    "    </div>",
      "  </section>",
      "</div>"
  ...
  function handleActionClick(appElement, button) {
    ...
    if (action === "load-release-example") {
      void loadLatestReleaseExample();
      return;
    }
  ...
  async function replaceProjectFromImportedData(parsedProject) {
    var hydratedProject = Utils.hydrateProjectImageAssets(parsedProject);
    var projectWithDefaults = Schema.normalizeProject(hydratedProject);
    var normalizedProject = await Utils.normalizeProjectImageAssets(
      projectWithDefaults,
      getImageImportOptions(projectWithDefaults)
    );
    await mountedStore.replaceProject(normalizedProject);
    await mountedStore.updateUi(function (ui) {
      ui.editingColorSequenceId = null;
      ui.selectedColorSequenceId = null;
      ui.selectedTokenId = null;
      ui.selectedComponentType = null;
