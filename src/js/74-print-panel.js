(function (global, factory) {
  ...
  function renderPageSvg(page, project, isPreview) {
    ...
    return [
      ...
      page.items.map(function (item) {
        return renderPageCellBackground(item, project);
      }).join(""),
      page.items.map(function (item) {
  ...
  function renderPageCellBackground(item, project) {
    if (item.cellSizeIn <= item.diameterIn) {
      return "";
    }
    var face = item.token.front;
    var fill = getPageCellFill(face, project.sequences.color, item.sequenceIndex);
    var cellX = item.cellXIn * 100;
    var cellY = item.cellYIn * 100;
    var cellSize = item.cellSizeIn * 100;
  ...
  function renderCutlineTicks(cellX, cellY, cellSize, tokenX, tokenY, tokenSize, inset, color) {
    var lineWidth = 1;
    var leftX = tokenX;
    var rightX = tokenX + tokenSize;
    var topY = tokenY;
    var bottomY = tokenY + tokenSize;
    return [
      renderCutlineTick(leftX, cellY, leftX, cellY + inset, color, lineWidth),
      renderCutlineTick(rightX, cellY, rightX, cellY + inset, color, lineWidth),
      renderCutlineTick(leftX, cellY + cellSize - inset, leftX, cellY + cellSize, color, lineWidth),
      renderCutlineTick(rightX, cellY + cellSize - inset, rightX, cellY + cellSize, color, lineWidth),
      renderCutlineTick(cellX, topY, cellX + inset, topY, color, lineWidth),
      renderCutlineTick(cellX, bottomY, cellX + inset, bottomY, color, lineWidth),
      renderCutlineTick(cellX + cellSize - inset, topY, cellX + cellSize, topY, color, lineWidth),
      renderCutlineTick(cellX + cellSize - inset, bottomY, cellX + cellSize, bottomY, color, lineWidth)
    ].join("");
  }
